...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        from exercise_code.classifiers.softmax import stable_softmax
        
        p = stable_softmax(scores)
        log_likelihood = -np.log(p[range(N),y])
        loss = np.sum(log_likelihood) / N
        loss += 0.5*reg*np.sum(np.square(W1)) #L2 Regularization for W1
        loss += 0.5*reg*np.sum(np.square(W2)) #L2 Regularization for W2
        ...
        grads['b2'] = np.dot(d2.T, np.ones(N))
        
        grads['W2'] = np.dot(hidden.T,d2)
        grads['W2'] += reg*W2

        d1 = mask*np.dot(p,W2.T)
        
        grads['b1'] = np.dot(d1.T,np.ones(N))
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    for learning_rate in [1e-3]: 
        for hidden_size in [150, 180, 200]: 
            for reg in [0.4, 0.6]:
                for num_iters in [1000]:
                    for batch_size in [220, 270]:
                        ...
                        print('with params: ', (learning_rate, hidden_size, reg, num_iters, batch_size))
                        if val_acc > best_acc:
                            best_acc = val_acc
                            best_params[(learning_rate, hidden_size, reg, num_iters, batch_size)] = val_acc
                            best_stats = stats
                            best_net = net
